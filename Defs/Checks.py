...
if checkConnection() == False:
    print('''{1}
        ...
        STRT STRT
        STRT STRT
     (This tools is not fully by STRT.Most of the cridets goes to DarkSecDevelopers.We only have just modified it. 
...
def checkPermissions():
    ...
    if systemos() != "Windows":
        if getuid() == 0:
            ...
            print(
                "{0}Permissions denied! Please run as '{1}sudo{0}'(Means first run sudo su in your terminal and enter into HiddenEye)".format(RED, GREEN))
            exit()
    else:
